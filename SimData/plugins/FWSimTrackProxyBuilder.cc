...
class FWSimTrackProxyBuilder : public REveDataSimpleProxyBuilder {
public:
...
private:
  ...
  void BuildItem(const void* data, int index, REveElement* iCollectionHolder, const REveViewContext*) override
  {
    printf("this should not be called \n");
  }

  using REveDataSimpleProxyBuilder::BuildItemViewType;
  void BuildItemViewType(const void* data, int index, REveElement* iCollectionHolder,  const std::string& viewType, const REveViewContext*) override
  {
    printf("this should not be called \n");
  }
...
//______________________________________________________________________________
  void FWSimTrackProxyBuilder::BuildProduct(const REveDataCollection *collection, REveElement *product, const REveViewContext *)
  {
  ...
  auto event = context->getCurrentEvent();
  event->getByLabel(edm::InputTag("g4SimHits"), hitColl);

  int i = 0;
  for (int index = 0; index < collection->GetNItems(); ++index)
  {
      const REveDataItem* di = collection->GetDataItem(index);
      if (di->GetRnrSelf() && !di->GetFiltered())
      {
          SimTrack* iDataPtr = (SimTrack*)(collection->GetDataPtr(index));
          SimTrack& iData = *iDataPtr;
          double vx = 0.0;
          double vy = 0.0;
          double vz = 0.0;
          double vt = 0.0;
          ...
          REveTrack* track = new REveTrack(particle, ++i, propagator);
          if (iData.charge() == 0) {
          ...
          SetupAddElement(track, itemHolder);
      }
  }
}

REGISTER_FW2PROXYBUILDER_BASE(FWSimTrackProxyBuilder,edm::SimTrackContainer, "SimTracks");